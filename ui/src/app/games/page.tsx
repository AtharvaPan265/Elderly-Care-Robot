...
const GameButton: React.FC<GameButtonProps> = ({ title, description, href, Icon }) => (
    <Link 
        href={href} 
        className={`
            flex flex-col items-center justify-center text-center // Ensures vertical and horizontal centering
            p-6 bg-white rounded-2xl shadow-xl transition-all duration-300 h-64 // Added h-48 for uniform size
    ...
    return (
        <div className="bg-[#e0f7fa] min-h-screen p-6 md:p-10 flex justify-center">
            <div className="w-full max-w-5xl bg-white p-8 rounded-3xl shadow-2xl">
                <Link href="/dash" className="text-gray-700 hover:text-gray-900 flex items-center mb-10 font-semibold transition">
                    <IoIosArrowBack size={24} className="mr-1" />
                    Back to Dashboard
                </Link>
                <h1 className="text-4xl font-extrabold text-gray-900 mb-8 border-b-2 border-blue-100 pb-3">
