    ...
    return (
        <Link 
            ...
            className={`
                ...
                h-20 font-bold cursor-pointer transition-transform duration-150 
        ...
        >
            <Icon size={30} />
            <div className="text-lg text-left">{label}</div>
        </Link>
    );
}
...
const DailySchedulePanel: React.FC = () => {
    ...
    // if (profile.medicationList.length === 0) {
    //     useUserProfile().updateMedicationList([
    ...
    return (
        <div className={`p-6 rounded-3xl shadow-2xl w-full h-full border border-gray-200 flex flex-col justify-between`} style={{ backgroundColor: PARCHMENT_LIGHT }}>
            <h2 className="text-3xl font-extrabold text-gray-900 mb-6 border-b-2 border-orange-100 pb-2">
                Daily Schedule
            </h2>
            <div className="mb-8 max-h-[60vh] overflow-y-auto">
                <h3 className={`text-xl font-bold text-gray-700 mb-3 border-l-4 border-[${BLUE_ACCENT}] pl-2`}>Hourly Agenda</h3>
                <ul className="space-y-2">
                    {dailyEventsSorted.map((item, index) => (
                        <li key={index} className="flex items-center text-base border-b border-gray-100 pb-1">
                            <span className="font-semibold w-1/4 min-w-[80px] text-gray-500">{item.date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true })}</span>
                            <span className={`font-medium ${item.color} ml-4`}>{item.title}</span>
                        </li>
                    ))}
                </ul>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-t pt-4">
                <div className="bg-white p-4 rounded-xl shadow-md border border-green-100">
                    <h3 className={`text-lg font-bold mb-3 flex items-center text-[${GREEN_ACCENT}]`}>
                        <FaPills className="mr-2" /> Today's Medications
                    </h3>
                    ...
                    <div className="p-6 bg-white rounded-2xl shadow-xl text-center">
                        <div className="flex justify-between items-center mb-3">
                            <div className="flex flex-col items-start">
                                <div className="text-2xl font-bold">{isLoaded ? timeStr : 'Loading...'}</div>
                                <span className="text-xl font-normal text-gray-600">{isLoaded ? dateStr : '...'}</span>
                            </div>
